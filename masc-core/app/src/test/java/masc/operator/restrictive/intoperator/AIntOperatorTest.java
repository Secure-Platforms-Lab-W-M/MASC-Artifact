...
public abstract class AIntOperatorTest {
    ...
    public void absoluteValue() {
        ...
        assertEquals(expected,
                new AbsoluteValue(p).mutation().replace("%d", ""));
    }
    ...
    public void iterationMultipleCall() {
        ...
        expected += "\n}";
        assertEquals(expected,
                new IterationMultipleCall(p).mutation().replace("%d", ""));
    }
    ...
    public void valueInVariable() {
        ...
        assertEquals(expected,
                new ValueInVariable(p).mutation().replace("%d", ""));
    }
    ...
    public void valueInVariableArithmetic() {
        ...
        assertEquals(expected,
                new ValueInVariableArithmetic(p).mutation().replace("%d", ""));
    }
    ...
    public void whileLoopAccumulation() {
        ...
        assertEquals(expected,
                new WhileLoopAccumulation(p).mutation().replace("%d", ""));
    }

    @Test
    public void overflow() {
        expected = String.format(template,
                "Integer.MAX_VALUE + Integer.MAX_VALUE + 2 + 50");
