...
public class PublicIPResolver
{
        ...
        if (data.length >= 8) {
            ...
            if (family == 1) {
                ...
                String ip = firstOctet +
                        ...
                        "." + thirdOctet +
                        "." + fourthOctet;
                return new Header(new InetSocketAddress(ip, port), type, lengthValue + 4);
            }
        }
    ...
    public static InetSocketAddress getPublicAddress(final String stunServer, final int port)
    {
        ...
        for (int t = 0; t < 3; t++) {
            try {
                final SelDatagramChannel channel = SelDatagramChannel.open(null,
