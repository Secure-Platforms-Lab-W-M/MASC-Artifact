...
public class ConfiguredObjectRegistrationImpl implements ConfiguredObjectRegistration
{
    ...
    public ConfiguredObjectRegistrationImpl()
    {
        ...
        if(!Boolean.getBoolean("qpid.type.disabled:virtualhostnode.BDB_HA"))
        {
             implementations.add(BDBHAVirtualHostNodeImpl.class);
        }
        if(!Boolean.getBoolean("qpid.type.disabled:virtualhostnode.BDB"))
        {
             implementations.add(BDBVirtualHostNodeImpl.class);
        }
        implementations.add(BDBHARemoteReplicationNode.class);
        _implementations = Collections.unmodifiableSet(implementations);
    }

    public String getType()
    {
        return "org.apache.qpid.server.virtualhostnode.berkeleydb";
    }
