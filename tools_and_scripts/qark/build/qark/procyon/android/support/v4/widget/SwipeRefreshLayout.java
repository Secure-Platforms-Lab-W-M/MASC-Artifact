...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild
{
    ...
    private void moveSpinner(final float n) {
        ...
        final float n4 = (float)mSpinnerOffsetEnd;
        final float max = Math.max(0.0f, Math.min(abs - mTotalDragDistance, n4 * 2.0f) / n4);
        final double n5 = max / 4.0f;
        final double pow = Math.pow(max / 4.0f, 2.0);
        Double.isNaN(n5);
        final float n6 = (float)(n5 - pow) * 2.0f;
        ...
        this.mProgress.setProgressRotation((0.4f * n3 - 0.25f + 2.0f * n6) * 0.5f);
        this.setTargetOffsetTopAndBottom(mOriginalOffsetTop + n7 - this.mCurrentTargetOffsetTop);
    }
    ...
    protected void onLayout(final boolean b, int measuredWidth, int n, int n2, int paddingTop) {
        ...
        paddingTop = measuredWidth / 2;
        final int n3 = n / 2;
        final int mCurrentTargetOffsetTop = this.mCurrentTargetOffsetTop;
        mCircleView.layout(paddingTop - n3, mCurrentTargetOffsetTop, measuredWidth / 2 + n / 2, mCurrentTargetOffsetTop + n2);
    }
