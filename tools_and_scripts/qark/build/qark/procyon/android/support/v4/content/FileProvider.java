...
public class FileProvider extends ContentProvider
{
    ...
    private static PathStrategy getPathStrategy(final Context context, final String s) {
        while (true) {
            ...
            synchronized (FileProvider.sCache) {
                pathStrategy = FileProvider.sCache.get(s);
                if (pathStrategy == null) {
                    PathStrategy pathStrategy2;
                    try {
                        pathStrategy2 = parsePathStrategy(context, s);
                        FileProvider.sCache.put(s, pathStrategy2);
                    }
    ...
    public Cursor query(final Uri uri, String[] columns, final String s, String[] array, String s2) {
        final File fileForUri = this.mStrategy.getFileForUri(uri);
        if (columns == null) {
            columns = FileProvider.COLUMNS;
        }
        array = new String[columns.length];
        final Object[] array2 = new Object[columns.length];
        int n = 0;
        for (int length = columns.length, i = 0; i < length; ++i) {
            s2 = columns[i];
            if ("_display_name".equals(s2)) {
                array[n] = "_display_name";
                array2[n] = fileForUri.getName();
                ++n;
            }
            else if ("_size".equals(s2)) {
                array[n] = "_size";
                array2[n] = fileForUri.length();
                ++n;
            }
        }
        final String[] copy = copyOf(array, n);
        final Object[] copy2 = copyOf(array2, n);
            ...
            try {
                ...
                for (final Map.Entry<String, File> entry : this.mRoots.entrySet()) {
                    ...
                    if (canonicalPath.startsWith(path)) {
                        if (string != null && path.length() <= ((Map.Entry<K, File>)string).getValue().getPath().length()) {
                            continue;
                        }
                        string = (File)entry;
                    }
