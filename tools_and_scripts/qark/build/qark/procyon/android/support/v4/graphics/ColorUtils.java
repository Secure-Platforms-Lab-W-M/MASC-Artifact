...
public final class ColorUtils
{
    ...
    public static void RGBToHSL(@IntRange(from = 0L, to = 255L) final int n, @IntRange(from = 0L, to = 255L) final int n2, @IntRange(from = 0L, to = 255L) final int n3, @NonNull final float[] array) {
        ...
        float n13 = 60.0f * n10 % 360.0f;
        if (n13 < 0.0f) {
            n13 += 360.0f;
        }
        array[0] = constrain(n13, 0.0f, 360.0f);
    ...
    public static void RGBToXYZ(@IntRange(from = 0L, to = 255L) final int n, @IntRange(from = 0L, to = 255L) final int n2, @IntRange(from = 0L, to = 255L) final int n3, @NonNull final double[] array) {
        if (array.length == 3) {
            final double n4 = n;
            Double.isNaN(n4);
            final double n5 = n4 / 255.0;
            double pow;
            if (n5 < 0.04045) {
                pow = n5 / 12.92;
            }
            else {
                pow = Math.pow((n5 + 0.055) / 1.055, 2.4);
            }
            final double n6 = n2;
            Double.isNaN(n6);
            final double n7 = n6 / 255.0;
            double pow2;
            if (n7 < 0.04045) {
                pow2 = n7 / 12.92;
            }
            else {
                pow2 = Math.pow((n7 + 0.055) / 1.055, 2.4);
            }
            final double n8 = n3;
            Double.isNaN(n8);
            final double n9 = n8 / 255.0;
            double pow3;
            if (n9 < 0.04045) {
                pow3 = n9 / 12.92;
            }
            else {
                pow3 = Math.pow((0.055 + n9) / 1.055, 2.4);
            }
    ...
    public static double calculateContrast(@ColorInt int compositeColors, @ColorInt final int n) {
        if (Color.alpha(n) == 255) {
            ...
            final double n3 = calculateLuminance(n) + 0.05;
    ...
    private static double[] getTempDouble3Array() {
        final double[] array = ColorUtils.TEMP_ARRAY.get();
        if (array == null) {
            final double[] array2 = new double[3];
            ColorUtils.TEMP_ARRAY.set(array2);
            return array2;
        }
        return array;
    }
