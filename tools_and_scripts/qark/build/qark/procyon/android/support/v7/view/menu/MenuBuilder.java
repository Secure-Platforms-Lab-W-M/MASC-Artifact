...
public class MenuBuilder implements SupportMenu
{
    ...
    MenuItemImpl findItemWithShortcutForKey(final int n, final KeyEvent keyEvent) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if ((c == keyCharacterMap$KeyData.meta[0] && (metaState & 0x2) == 0x0) || (c == keyCharacterMap$KeyData.meta[2] && (metaState & 0x2) != 0x0)) {
                return menuItemImpl;
            }
    ...
    void findItemsWithShortcutForKey(final List<MenuItemImpl> list, final int n, final KeyEvent keyEvent) {
        ...
        for (int size = this.mItems.size(), i = 0; i < size; ++i) {
            ...
            if ((modifiers & 0x1100F) == (0x1100F & n2) && c != '\0' && (c == keyCharacterMap$KeyData.meta[0] || c == keyCharacterMap$KeyData.meta[2] || (qwertyMode && c == '\b' && n == 67))) {
    ...
    public boolean performItemAction(final MenuItem menuItem, final MenuPresenter menuPresenter, final int n) {
        ...
        if (menuItemImpl.hasCollapsibleActionView()) {
            ...
            if (menuItemImpl.hasSubMenu() || b) {
                ...
                final boolean b3 = invoke | this.dispatchSubMenuSelected(subMenuBuilder, menuPresenter);
                if (!b3) {
                    this.close(true);
                }
                return b3;
